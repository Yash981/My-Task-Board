...
const TaskDetails = (props: Props) => {
  ...
  const handleStatusSelect = (
    value: "IN_PROGRESS" | "COMPLETED" | "WONT_DO"
  ) => {
    setValue("status", value);
  };

  const isPending =
    editTask.isPending || deleteTask.isPending || createTask.isPending;
  const onSubmit = (formData: FormValues) => {
    // console.log(formData,params?.id)
    const mappedData = {
      title: formData.title,
      desc: formData.desc ?? "",
      icon: formData.icon as
        | "COFFEE_CUP"
        | "OFFICE_WORK"
        | "MESSAGE"
        | "WEIGHT_LIFT"
                      ...
                      <FormControl>
                        <div className="flex space-x-4">
                          {icons.map((icon) => (
                            <div
                              key={icon.value}
                              className={`cursor-pointer  p-0.5 rounded-xl ${
                                field.value === icon.value
                                  ? "border-2 border-[#3662E3]"
                                  : ""
                              }`}
                              onClick={() =>
                                handleIconSelect(
                            ...
                            >
                              <Image
                                src={icon.src}
                                alt={icon.label}
                                className={`w-10 h-10 rounded-lg inline-block bg-gray-200   p-2 
                      ...
                      <FormControl>
                        <div className="grid grid-cols-2 gap-4 ">
                          {status.map((icon) => (
                            <div
                              key={icon.value}
                              className={`cursor-pointer  p-0.5  md:w-full rounded-xl flex items-center lg:pr-20  ${
                                field.value === icon.value
                                  ? "border-2 border-[#3662E3]"
                            ...
                            >
                              <Image
                                src={icon.src}
                                alt={icon.label}
                                className={`w-10 h-10 rounded-lg inline-block ${
                                  icon.value === "WONT_DO" ? "bg-red-500" : ""
                                }
                                                    ${
                                                      icon.value === "COMPLETED"
                                                        ? "bg-green-500"
                                                        : ""
                                                    }
                                                    ${

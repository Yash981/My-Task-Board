...
const FetchEachTask = ({ task }: Props) => {
    ...
    return (
        <div>
            <Link href={`/board/${id}`}>
                <div className={`w-full  p-4 ${status === 'WONT_DO' ? 'bg-[#F7D4D3]' : ""} ${status === 'COMPLETED' ? 'bg-[#A0ECB1]' : ""} ${status === 'IN_PROGRESS' ? 'bg-[#F5D565]' : ""}  ${status === 'TODO' ? 'bg-[#E3E8EF]' : ""}  rounded-2xl flex items-center gap-3 float-left cursor-pointer justify-between `} onClick={() => onOpen()}>
